...
try {
    ...
    if (!tabsContainer) {
        console.error('CRITICAL: tabs-container not found in DOM!');
        alert('Critical Error: tabs-container missing');
    }
    ...
    ipcRenderer.on('blackbox-created', () => {
        console.log('Renderer: blackbox-created received');
        ...
        if(tabsContainer) {
            ...
            console.error('Cannot add Blackbox: tabsContainer missing');
        }
    });

    function createTab(id) {
        console.log('Renderer: creating tab element for', id);
        ...
        if(tabsContainer) {
            ...
            console.log('Renderer: Tab element added to DOM');
        } else {
            console.error('Renderer: tabsContainer not found! Cannot add tab.');
            alert('Error: Cannot draw tab (DOM missing)');
        }
    }
    ...
    ipcRenderer.on('tab-created', (e, id) => {
        console.log('Renderer: tab-created event received for', id);
        try {
            createTab(id);
        } catch (err) {
            console.error('Renderer: Failed to create tab UI', err);
            alert('Failed to create tab UI: ' + err.message);
        }
    });
    ...
    console.log('Renderer script loaded successfully. Sending ready signal...');
    ipcRenderer.send('renderer-ready');

} catch (err) {
    console.error('Renderer Script Error:', err);
    alert('Renderer Fatal Error: ' + err.message);
}
